...
└── <video_name_N>
    ...
    └── label.json 
```
**Note**: Approximately 50% of the data were filtered out by the data cleaning pipeline and therefore do not have annotations The filtered trajectories are listed in `filtered_trajs.txt` (generated by `scripts/merge_annotations.py`) and can be safely deleted to free up storage space.
